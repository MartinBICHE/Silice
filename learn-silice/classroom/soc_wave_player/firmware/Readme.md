...
- Possibilité de passer à la musique suivante ou précèdente directement.

### Mode de fonctionnement

Le pilotage s'effectue via les boutons B1 à B6 :

- B1 : Mettre la musique en pause ou reprendre la lecture.
- B2 : Arrêter la musique et retourner au menu principal.
...
- B5 : Retourner au dossier parent dans le menu. Pendant la musique : passage au titre précèdent (appui court) ou retour rapide (appui long).
- B6 : Sélectionner un dossier ou une musique. Pendant la musique : passage au titre suivant (appui court) ou avance rapide (appui long).
...
Pour charger le projet de manière permanente sur la carte (Flash) :
`openFPGALoader -f -b ulx3s BUILD_final/build.bit`

### Limites du projet

- La gestion des jeux de lumière est effectuée par le firmware (je me suis rendu compte trop tard que c'était au harware de s'en occuper).
...
- Des grisillements sonores légers persistent malgré les optimisations, notamment lors des accès a la carte SD.